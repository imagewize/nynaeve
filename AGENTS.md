...
- ⚠️ In **content markup** the block is always `<!-- wp:imagewize/x -->`. All 36 blocks use the `imagewize` namespace; `nynaeve` is a *category* prefix only and never appears in serialized content. `wp:nynaeve/x` is always a bug — it renders as an unsupported-block placeholder. (Found Aug 2026: bad docs propagated `wp:nynaeve/cta-block-blue` across a whole content repo.) Verify a slug with `grep '"name"' resources/js/blocks/*/block.json`.
